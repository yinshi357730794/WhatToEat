...
@interface YSHeartView ()
{
    CGFloat t;
}

@property(nonatomic) BOOL isSelfSelected;

@end

@implementation YSHeartView
...
- (instancetype)initWithFrame:(CGRect)frame
{
    ...
    if (self) {
        //[self loadTimer];
        [self addTapRecognizer:self action:@selector(selfDidPressed)];
    }
    return self;
}
...
- (void)setHeartStrokeColor
{
    ...
    if (self.strokeColor) {
        ...
        if (self.changeStrokeColorWhenFillUp) {
            strokColor = self.fillColor;
        } else {
            strokColor = self.strokeColor;
        }
    }else{
...
-(void)selfDidPressed{
    ...
    self.isSelfSelected = !self.isSelfSelected;
    
    [self setNeedsDisplay];
    NSLog(@"self did pressed ");
